        ...
        <Card className="group bg-card border-border hover:shadow-lg transition-all duration-300">
          <CardContent className="p-5">
            <Link to={`/products/${logic.product.slug}`} className="block">
              <div className="aspect-square bg-muted rounded-lg mb-4 overflow-hidden relative">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2">
                  {logic.discountPercentage && (
                    <span className="bg-destructive text-destructive-foreground text-xs px-2.5 py-1 rounded-full font-semibold shadow-sm">
                      -{logic.discountPercentage}%
                    </span>
                  )}
                  {logic.product.featured && (
                    <span className="bg-accent text-accent-foreground text-xs px-2.5 py-1 rounded-full font-semibold shadow-sm">
                      Destacado
                    </span>
                  )}
                  {!logic.inStock && (
                    <span className="bg-muted text-muted-foreground text-xs px-2.5 py-1 rounded-full font-semibold shadow-sm">
                      Agotado
                    </span>
                  )}
                </div>
              ...
              <h3 className="font-semibold text-base text-card-foreground mb-2 line-clamp-2">
                {logic.product.title}
              </h3>
            ...
            {logic.hasVariants && logic.options && (
              <div className="mb-4 space-y-3">
                {logic.options.map((opt) => (
                  <div key={opt.id}>
                    <div className="text-xs font-semibold text-card-foreground mb-2">{opt.name}</div>
